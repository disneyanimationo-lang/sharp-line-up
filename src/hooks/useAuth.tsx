...
interface AuthContextType {
  ...
  signUp: (email: string, password: string, role: 'customer' | 'shop_owner', name: string) => Promise<{ error: any }>;
...
export const AuthProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(
      (event, session) => {
        ...
        if (session?.user) {
          setTimeout(() => {
            checkShopOwnerRole(session.user.id);
          }, 0);
        } else {
  ...
  const checkShopOwnerRole = async (userId: string) => {
  ...
  const signUp = async (email: string, password: string, role: 'customer' | 'shop_owner', name: string) => {
